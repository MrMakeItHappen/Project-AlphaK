...
final class ServiceProvider: NSObject {
    ...
    func serviceRequest(typeOfRequest: TypeOfRequest, passedParameters : [String : Any]?, endpoint: String, requiresAuth : Bool, completion: @escaping ([String: Any]?, Error?) -> Void) {
        ...
        if requiresAuth {
            if _jwtToken != nil {
                Printer().print(message: "🔵 _jwtToken added to the request")
                request.addValue("Bearer \(_jwtToken!)", forHTTPHeaderField: "Authorization")
            } else {
                Printer().print(message: "🔴 _jwtToken is nil but request requires authentication")
            }
        }
